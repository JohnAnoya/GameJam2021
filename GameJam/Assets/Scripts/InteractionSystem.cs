    ...
    void CheckForInteractions()
    {
        ...
        if (Physics.Raycast(ray, out hit, 1.0f))
        {
            ...
            else if (hit.transform.tag == "Note2")
            {
                ...
                {
                    showingPopup = true;
                    tempPopup = Instantiate(InteractionPopUp, new Vector3(hit.transform.position.x, hit.transform.position.y + 0.5f, hit.transform.position.z - 0.4f), Quaternion.identity);
                    tempPopup.GetComponentInChildren<TMP_Text>().SetText("Pick up note");
                }
